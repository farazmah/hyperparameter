...
class Hyperparemeter:
    ...
    def optimize(self, trials, score, added_evals):
        ...
        space = {
            ...
            'min_sum_hessian_in_leaf': hp.quniform('min_sum_hessian_in_leaf', 0.4, 0.9, 0.05),
    ...
    def tune_model(self, ds_x, ds_y, fold, trials):
        """
        ...
        :param ds_y: pandas.Dataframe
            Training label
        :param fold: sklearn model_selection or cross_validation object
        ...
        if trials is None:
            trials = Trials()
            additional_evals = 100
        else:
            additional_evals = len(trials.losses()) + 10

        # Create the loss function
        loss_func = create_loss_func(ds_x, ds_y, folds)

        # Find optimal hyperparameters
        parameters = optimize(trials, loss_func, additional_evals)
